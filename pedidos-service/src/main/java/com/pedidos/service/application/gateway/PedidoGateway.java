...
public class PedidoGateway {
    ...
    public PedidoDTO consultarPeloIdentificador(UUID identificador) throws PedidoNotFoundException {
    ...
    public List<PedidoDTO> gerarRelatorioPedidosPagos() throws GestaoDePedidosApplicationException {
        return this.pedidoUseCaseContract.gerarRelatorioPedidosPagos().stream().map(Pedido::toDTO).toList();
    }

    public PedidoDTO liquidarPedido(UUID identificador) throws GestaoDePedidosApplicationException {
        Pedido pedido = this.pedidoUseCaseContract.liquidarPedido(identificador);
        return pedido.toDTO();
    }

    public PedidoDTO enviar(UUID identificador) throws GestaoDePedidosApplicationException {
        Pedido pedido = this.pedidoUseCaseContract.enviar(identificador);
        return pedido.toDTO();
    }

    public PedidoDTO entregar(UUID identificador) throws GestaoDePedidosApplicationException {
